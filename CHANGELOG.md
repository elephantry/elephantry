...
- Derive `FromSql`/`ToSql` for enums instead of `FromText`/`ToText`;
...
### Changed
- bit-vec 0.9;
- `serde` feature also enable optionnal dependencies `serde` feature;
...
- `inspect::Constraint::NotNull` variant;
- `Notify` type;
...
### Fixed
- [Broken enum derive macro](https://github.com/elephantry/elephantry/pull/37).
